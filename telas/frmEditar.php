...
echo '<h1>'.$res['jogo']['nomej'].'</h1>';
    ...
    ?>
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" id="texto" rows="5" > <?php echo $res['existe'] ? $res['analise']['texto'] : "" ?> </textarea>
          <span id="num"></span>
        </div>

        <div class="form-group">
          <label for="formControlRange">História</label>
          <input type="range" class="form-control-range" id="historia" min="0" max="100"  value="<?php echo $res['existe'] ? $res['analise']['nHistoria'] : "0" ?>">
          <span id="valorH"><?php echo $res['existe'] ? $res['analise']['nHistoria'] : "0" ?></span>
        </div>

        <div class="form-group">
          <label for="formControlRange">Gráficos</label>
          <input type="range" class="form-control-range" id="graficos" min="0" max="100"  value="<?php echo $res['existe'] ? $res['analise']['nGraficos'] : "0" ?>">
          <span id="valorG"><?php echo $res['existe'] ? $res['analise']['nGraficos'] : "0" ?></span>
        </div>

        <div class="form-group">
          <label for="formControlRange">Jogabilidade</label>
          <input type="range" class="form-control-range" id="jogabilidade" min="0" max="100"  value="<?php echo $res['existe'] ? $res['analise']['nJogabilidade'] : "0" ?>">
          <span id="valorJ"><?php echo $res['existe'] ? $res['analise']['nJogabilidade'] : "0" ?></span>
        </div>

        <div class="form-group">
          <label for="formControlRange">Performance</label>
          <input type="range" class="form-control-range" id="performance" min="0" max="100"  value="<?php echo $res['existe'] ? $res['analise']['nPerformance'] : "0" ?>">
          <span id="valorP"><?php echo $res['existe'] ? $res['analise']['nPerformance'] : "0" ?></span>
        </div>

        <button class="btn btn-primary" type="button" id="confirmarEdt">Confirmar Edição</button>
